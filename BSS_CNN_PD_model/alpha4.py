# -*- coding: utf-8 -*-
"""
Created on Sun Jul  1 16:59:19 2018 Updated in Jul 16
CNN-超声频谱识别软件 - main based on Tensorflow 
...
# =============================================================================
EPOCH = 100               # 迭代次数
IMAGE_MUMBER = 490       # 训练集总数量
BATCH_SIZE = 14          # 每一批的载入数量 IMAGE_MUMBER / BATCH_SIZE 需要被整除      
...
CHAR_SET_LEN = 4 #output layer 输出胞元数量 （数据有多少种，即噪声种类数）
# =============================================================================
'相关函数'
...
# 3rd fully connected layer / output layer -- 1024 -> 4
...
for epoch in range(EPOCH):
    for step in range(int(IMAGE_MUMBER / BATCH_SIZE)):
        ...
        _, loss_ = sess.run([train_op,loss], feed_dict = {tf_x: b_x, tf_y: b_y, keep_prob: DROPOUT_RATE})
        if (step * BATCH_SIZE) % (BATCH_SIZE * 2) == 0:
            ...
            accuracy_, flat_representation = sess.run([accuracy, fc2], {tf_x: t_x, tf_y: t_y, keep_prob:1})
            if step % 5 == 0:
                print('Epoch:', epoch, '| Step:', step * BATCH_SIZE, '| train loss: %.5f' %loss_, '| test accuracy: %.3f ' % accuracy_)
...
test_output = sess.run(output, {tf_x: t_x[:15], keep_prob:1})
